...
parser.add_argument('-i', '--input_filepath', help='the file holding the Accurity CNV result')
parser.add_argument('-o', '--output_filepath', default='None', help='the output png full path, i.e. output/cnv.png')
args = parser.parse_args()

chr_len = (249250621, 243199373, 198022430, 191154276, 180915260, 171115067, 159138663, 146364022, 141213431, 135534747,
...
def plotSegment(ax, df, ylabel, ymax):
	lines = []
	c = []
...
def plotGenomicSegment(data, output_filepath):
	# plot copy number
	ymax = 0
	for key in data:
	...
	plotRowIndex = 0
	if data.has_key("truth"):
		plotSegment(axes[plotRowIndex], data['truth'], "Ground Truth", ymax)
		plotRowIndex += 1
	if data.has_key("Accurity"):
		plotSegment(axes[plotRowIndex], data['Accurity'], 'Accurity', ymax)
		plotRowIndex += 1
