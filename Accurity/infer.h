...
class OnePeak {
   public:
    ...
    int no_of_snps;
    int no_of_maf_peaks;
    float snp_coverage_sum;
    float snp_coverage_squared_sum;
    float snp_coverage_var_sum;
    float snp_coverage_mean;
    double snp_coverage_var;
    float snp_maf_var;
    vector<int> segment_rc_ratio_vector;
    //probability density function of SNP maf(X RESOLUTION)
    vector<double> maf_int_pdf_vec;
    ...
    void ResetCounters() {
        no_of_snps = 0;
        no_of_maf_peaks = 0;
        snp_coverage_sum = 0.0;
        snp_coverage_squared_sum = 0.0;
        snp_coverage_var_sum = 0.0;
        snp_coverage_mean = 0.0;
        snp_coverage_var = 0.0;
        snp_maf_var = 0.0;
    }
...
class OnePeriod {
   public:
    ...
    double best_purity, best_ploidy, logL_rc, adj_logL_rc, logL_rc_penalty,
        best_logL_snp, best_lod_snp, best_adj_logL_snp, best_logL_snp_penalty,
...
class Infer {
   ...
   private:
    ...
    int output_snp_maf_by_segment();
    int output_snp_maf_by_peak(vector<OnePeak> &peak_obj_vector);
    ...
    double adjust_maf_expect(double major_allele_fraction_exp,
    ...
    double output_copy_number_segments(OnePeriod &best_period_obj,
                                    vector<OnePeak> &peak_obj_vector);
    void recalibrate_Q_and_purity_based_on_cnv_ploidy(OnePeriod &best_period_obj);
    ...
    ofstream _snp_maf_exp_vs_adj_outf;
